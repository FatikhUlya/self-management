...
export default function RulesPage() {
  const { state, addSelfRule, deleteSelfRule } = useLifeOS();
  ...
  const handleAddRule = async (e: React.FormEvent) => {
    ...
    await addSelfRule(newRule.trim());
    setNewRule('');
  };
        ...
        {rules.length === 0 ? (
          ...
          <div className="grid gap-3">
            {rules.map((rule) => (
              <Surface
                key={rule.id}
                className="p-4 flex items-center justify-between group hover:border-amber-500/30 transition-colors"
              >
